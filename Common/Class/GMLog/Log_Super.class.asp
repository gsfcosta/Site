...
<%
	...
	if (rCN = "LD1") Then
		...
		SQL = SQL & "	ifnull(b.ValueType,'-'), ifnull(b.Ahead,'-'), ifnull(b.after,'-'), 1 "
		SQL = SQL & " from   "
		SQL = SQL & " "&rServer&".TradeLog  as a left outer Join "
		SQL = SQL & " "&rServer&".TradeDetailLog as b "
	...
	Elseif (rCN = "LD2" ) Then
		...
		SQL = SQL & " LogType,LogSubType,ifnull(opt,'-'),ifnull(opt_level,'-'),ifnull(synergy,'-'),ifnull(synergy_level,'-'),DataType,1 as TableType From "

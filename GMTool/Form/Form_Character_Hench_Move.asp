...
<%
	...
	Dim rCItem		: 	rCitem			= Request("check_Hench")
...
<div style="padding-top: 15px">
	<center>
	<form id="form_Hench_Move<%=rGBN%>" action="/Common/Class/GMTool/GMT_Character_Hench.class.asp?I_Pro=HENHCMOVE" method="post">
	...
	<table>
		<tr>
			<td align=left><div style="padding-bottom:5px;">이동될 Hench, Server : <%=rServer%></div>
				<select id="MHench_sel" name="MHench_sel" size=15 multiple style="width:200px;"></select>
			</td>
			<td width="80">
	...
	<div style="padding-top:15px;"><span class="btn_pack medium icon"><span class="saveas"></span><button type="submit" id="char_Hench_move_btn">&nbsp;<%=MsgExtract("Btn_Save")%></button></span></div>
	</form>
	</center>
</div>		
...
$(function(){
	...
	for(var i = 0 ; i < a_check_item.length ; i++){
		s_option = s_option + '<option value="'+a_check_item[i][2]+'|'+a_check_item[i][1]+'">'+ s_monster_search(a_check_item[i][0]) +'</option>'
	}
	...
	$("#MHench_sel").empty().append(s_option) ;
	...
	$('#form_Hench_Move<%=rGBN%>').submit(function(){
		if (confirm('<%=MsgExtract("F9_Title030")%>')){
			$("#MHench_sel option").prop('selected',true);
			$(this).ajaxSubmit({ beforeSubmit: check_move_form, success: rtn_Move_Hench_val });
			return false;
		}else{
			return false;
		}
	...
	function rtn_Move_Hench_val(responseText, statusText, xhr, $form)  {  

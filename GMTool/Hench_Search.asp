...
<div id="wrap">
	...
	<div id="container" style="padding-bottom:10px;">
		...
		<div class="progress">
				...
				<ol><li class="on"><span><%=Now_Position%></span></li><li class="on"><span><%=MsgExtract("TOOL_Mid2_Title")%></span></li><li class="on"><span><%=MsgExtract("TOOL_SM4_Title")%></span></li></ol>
		</div>
		<h3 style="margin:10px 0 0 0; padding:0"><%=MsgExtract("TOOL_SM4_Title")%></h3> 
					...
					<td colspan="11">
						...
						<div class="fr2 item" style="float:left;"><% Server_Select %>
							<select id="s_gbn" name="s_gbn">
								<option value="S" selected><%=MsgExtract("F8_Title011")%></option>
							</select>
...
<script language=javascript>
	//<![CDATA[	
	var Json_url = "/common/class/GMTool/GMT_Character_Hench.class.asp";
	$(function($){
		menu_control(1, 1);
	...
	function gen_table(data){
		...
		if (data.List.length !=0 ){
			...
			$.each(data.List, function(i,Hench){
			
				html = html + '<tr>';
				html = html + '<td align=center class="f_gre" rel="'+unescape(Hench.N1)+'">'+ unescape(Hench.N11) +'</td>'; 
				html = html + '<td align=center><a href="/GMtool/Character_Info2.asp?id_idx='+unescape(Hench.N1)+'&PlayerID='+unescape(Hench.N11)+'&Server='+server_select.toLowerCase()+'&Name='+Hench.N2+'&Socket='+Hench.N5+'" class="link" target="blank">'+ unescape(Hench.N2) +'</a></td>';
				html = html + '<td align=center>'+ Char_Type2(unescape(Hench.N3)) +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N4) +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N5) +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N6) +'</td>';
				html = html + '<td align=center>'+ Enum_data.PositionType[parseInt(unescape(Hench.N7))].N2  +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N8) +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N9) +'</td>';
				html = html + '<td align=center>'+ unescape(Hench.N10) +'</td><td></td></tr> ';
